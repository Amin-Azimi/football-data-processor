...
export const handler = async(event: APIGatewayEvent): Promise<APIGatewayProxyResult> => {
    ...
    try{
        const maybeMatchEventData = await ValidateSchema<MatchEvent>(MatchEventSchema, event.body);
        if(!maybeMatchEventData.success){
            console.error('Validation failed:', maybeMatchEventData.error);
            return response.error( 'Validation failed', 400, maybeMatchEventData.error);
        }
