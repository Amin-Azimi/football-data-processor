...
export const saveMatchEvent = async (
  ...
  console.log(
    `Match event data with ID: ${eventId} published to EventBridge successfully`,
  );
  return eventId;
};

export const enrichAndProcessPublishedMatchEvent = async (
  matchEvent: PublishedMatchEvent,
): Promise<boolean> => {
  ...
  console.log(
    `Processing enriched match event: ${JSON.stringify(enrichedMatchEvent)}`,
  );
  return true;
};

export const getMatchEventDataStatistics = async (
  matchId: string,
  eventType: MatchEventType,
): Promise<MatchEventDataStatisticsResult> => {
  console.log(
    `Fetching match event data statistics for matchId: ${matchId} and eventType: ${eventType}`,
  );
