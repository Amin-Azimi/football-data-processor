...
export const storeObject = async ( key: string, body: object): Promise<void> => {
    ...
    try {
        ...
        console.log(`Successfully stored object in S3 bucket ${RAW_EVENTS_BUCKET} with key ${key}`);
    } catch (error) {
        console.error(`Error storing object in S3 bucket ${RAW_EVENTS_BUCKET} with key ${key}:`, error);
        throw error;
    }
}